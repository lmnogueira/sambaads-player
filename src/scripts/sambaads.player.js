	...
	window.jwplayer(self.player).onBeforePlay(function(evt){
		...
		setTimeout(function(){
			if(self.currentPlaylistIndex != self.lastPlaylistIndex){
				...
				window.jwplayer(self.player).playAd("https://pubads.g.doubleclick.net/gampad/ads?sz=640x360&iu=/387067271/Homologacao/441394631/441394871&cust_params=duration%3D%26CNT_Position%3Dpreroll%26category%3Dbeleza%26CNT_PlayerType%3Dsingleplayer%26CNT_MetaTags%3Dteste&cmsid="+self.getCurrentVideo().dfp_partner_id+"&vid="+self.getCurrentVideo().hashed_code+"&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&url=[referrer_url]&description_url=[description_url]&correlator=[timestamp]");
			}
		},5);
	});
