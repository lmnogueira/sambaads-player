...
WidgetController.prototype.play = function(element) {
	...
	if(this.params.target == "blank"){
		...
		this.sendMessage("click", JSON.stringify({image: this.getCurrentItem().image, publisherId: this.publisherInfo.hash_code, tagList: String(this.getCurrentItem().tag_list), mediaId: this.getCurrentItem().media_id, description: this.getCurrentItem().description, title: this.getCurrentItem().title}));
	}
};
